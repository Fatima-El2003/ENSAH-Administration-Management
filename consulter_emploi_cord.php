...
<body>
    ...
    <section class="main-content">
        <h2 id="title">Création d'Emploi</h2>
        ...
        <div class="modules padding" id="GI1-S1">
            <?php
        ...
        if (isset($_POST['adr'])) {
            ...
            foreach ($elements as $jour => $heures) {
                foreach ($heures as $heure => $element) {
                    if (!empty($element)) {
                        // Ajoutez ici le code pour mettre à jour la table emploi avec les nouvelles valeurs
                    }
                }
            }

            echo "Les éléments ont été mis à jour avec succès dans la table emploi.";
        }

        $sqlState = $pdo->prepare('SELECT * FROM emploi');
        $sqlState->execute();

        $emplois = $sqlState->fetchAll(PDO::FETCH_ASSOC);
                ...
                foreach ($jours as $jour) {
                    ...
                    foreach ($heures as $heure) {
                        ...
                        foreach ($emplois as $emploi) {
                            if ($emploi['jour'] == $jour && $emploi['heure'] == $heure) {
                                echo $emploi['id_module'];
                                $found = true;
                                break;
                            }
                        }
